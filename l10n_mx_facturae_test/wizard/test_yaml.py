...
class test_yaml_facturae(osv.osv_memory):
    ...
    def test_factuare_run (self, cr, uid, ids, context=None):
        ...
        for file_name_xml, file_name_yml in files_names:
            ...
            t = threading.Thread(target=self.execute_test_yaml, name=('test_facturae' + file_name_yml), args = args)
            t.daemon = False
            t.start()
    ...
    def execute_test_yaml(self, cr_original, uid, ids, file_name_xml , file_name_yml):
        ...
        try:
            ...
            tools.convert_xml_import(cr, 'l10n_mx_facturae_pac_sf', fp_data , {}, 'init', False, assertion_obj)
            tools.convert_yaml_import(cr, 'l10n_mx_facturae_pac_sf', fp_test ,'test', {}, 'init', False, assertion_obj)
        finally:
